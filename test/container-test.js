...
test('updater function updates a set of containers', t => {
  const containers = [
    createContainer({ match: [/--value-1/], skip: [] }),
    createContainer({ match: [/--value-2/], skip: [] })
  ];
  const updateContainers = createUpdaterFn(containers);
  const root = postcss.parse('.test-value-1 { top: var(--value-1); } .test-value-2 { right: var(--value-2); }');
